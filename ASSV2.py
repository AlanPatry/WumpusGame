...
class RationalAgent( Agent ):
    ...
    def think( self, percept ):
        ...
        elif self.state.goldIsGrabbed:
            list = self.solve('get_out', self.state)
            for i in list:
                next_actions.append(i.action)
        elif not self.state.isMapSafelyExplored():
    ...
    def IDS( self, goal, initial_state ):
        ...
        max_depth = 0
        while True:
          closed_list =  {initial_state:0}
          # Create initial stack with the initial board.
          open_list = [ [initial_state] ]
          while open_list:
            self.count += 1
            # Extract the path at the top of the stack
            current_path = open_list.pop(0)
            # Get the board place of that path
            current_state = current_path[-1]
            current_state.set_cell(current_state.cell, VISITED)

            # For debugging purpose, uncomment the following two lines
            # print(current_board)
            # pause("Next")
            ...
            if goal == 'kill' and current_state.wumpusIsKilled:
                return current_path
            if goal == 'explo':
                if self.state.isSafe(current_state.cell):
                    if current_state.isNeighborsSafelyExplored(3):
                        print('a')
                        return current_path
                    elif current_state.isNeighborsSafelyExplored(6):
                        print('b')
                        return current_path
                    elif current_state.isNeighborsSafelyExplored(9):
                        print('c')
                        return current_path
                    elif current_state.isMapSafelyExplored():
                        print('d')
                        return current_path
            ...
            if len(current_path) < max_depth:
                # Get where we can go from here
                next_states = self.generate_next_states(current_state, goal)
                # Add the new paths (one step longer) into the stack
                for state in next_states:
                    if(goal == 'explo'):
                        if(state.isSure(state.cell)):
                            if state not in closed_list or closed_list[state] > len(current_path): # Avoid revisited boards!
                                closed_list[state] = len(current_path)
                                open_list.insert(0, current_path + [state])
                    else: 
                        if state not in closed_list or closed_list[state] > len(current_path): # Avoid revisited boards!
                                closed_list[state] = len(current_path)
                                open_list.insert(0, current_path + [state])
    ...
    def BFS( self, goal, initial_state ):
        ...
        while open_list:
            ...
            if goal == 'kill' and current_state.wumpusIsKilled:
                return current_path
            if goal == 'explo' and current_state.isMapSafelyExplored():
                return current_path
            if goal == 'get_out' and current_state.getPos() == (1,1):
                return current_path
            if goal == 'risk' and not self.state.isSure(current_state.cell):
                #if (self.heuristic(current_state, goal) < 29):
                    print("cost = ",self.heuristic(current_state, goal))
                    return current_path
            else:
                # Add the new paths (one step longer) into the stack
                for next_state in self.generate_next_states(current_state, goal):
                    if(goal == 'explo'):
                        if(next_state.isSure(next_state.cell)):
                            if next_state not in closed_list: # Avoid revisited boards!
                                closed_list.add(next_state)
                                open_list.insert(0, current_path + [next_state])
                    else: 
                        if next_state not in closed_list: # Avoid revisited boards!
                            closed_list.add(next_state)
                            open_list.insert(0, current_path + [next_state])
    ...
    def generate_next_states(self, state):
        ...
        if (state.goldIsGrabbed and state.posx == 1 and state.posy == 1):
            next_state = State('climb', state.size, state.direction, state.posx, state.posy, state.wumpusIsKilled, state.goldIsGrabbed, state.wumpusLocation, state.arrowInventory, state)
            next_states.append(next_state)
        return next_states
