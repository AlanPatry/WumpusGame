...
class RationalAgent( Agent ):
    ...
    """
    isLearningAgent = False
    LONGEST_PATH = 500 # Found in literature
    INFINITY = 1e30
    FOUND = -1
    ...
    def think( self, percept ):
        ...
        if percept.glitter:
            ...
            list = self.solve('kill', copy.deepcopy(self.state))
            for i in list:
                next_actions.append(i.action)
        elif self.state.goldIsGrabbed:
            list = self.solve('get_out', copy.deepcopy(self.state))
            for i in list:
                next_actions.append(i.action)
        elif not self.state.isMapSafelyExplored():
            list = self.solve('explo', copy.deepcopy(self.state))
            for i in list:
                next_actions.append(i.action)
        else:
            list = self.solve('risk', copy.deepcopy(self.state))
            for i in list:
                next_actions.append(i.action)
    ...
    def solve(self, goal, initial_state):
        ...
        while bound < MAX_PATH_LENGTH:
            print(bound)
            self.closed_list = {initial_state: 1} 
            t = self.subSearch(goal, current_path, 0, bound)
    ...
    def subSearch(self, goal, current_path, g, bound):
        ...
        if goal == 'kill' and current_state.wumpusIsKilled:
            return current_path
        elif goal == 'explo' and current_state.isMapSafelyExplored():
            return current_path
        elif goal == 'get_out' and current_state.posx == 1 and current_state.posy == 1:
            return current_path
        elif goal == 'risk' and self.state.worldmap[current_state.posx][current_state.posy] == PITP:#not self.state.isSure(current_state.posx, current_state.posy):
            return current_path
        min = self.INFINITY
        for next_state in self.generate_next_states(copy.deepcopy(current_state)):
            alreadyClosed_List = next_state in self.closed_list and self.closed_list[next_state] <= g + 1
            if next_state not in current_path and not alreadyClosed_List:
                self.closed_list[next_state] = g + 1
                # Calcul du coût de l'état suivant
                next_cost = len(current_path) + self.heuristic(next_state)
                """if self.count > 0:
                ...
                current_path1 = current_path + [(next_state)]
                t = self.subSearch(goal, current_path1, g + 1, bound)
                if isinstance(t, (list,)):
                    return t
                if t < min:
                    min = t
